...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@ %p; %zi inserts; %lu deletes; %lu updates; %lu moves>",
            NSStringFromClass(self.class), self, (long)self.inserts.count, (unsigned long)self.deletes.count, (unsigned long)self.updates.count, (unsigned long)self.moves.count];
}
